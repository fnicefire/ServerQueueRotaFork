...
public class SystemCommand implements CommandExecutor {
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        if (!sender.hasPermission("queue.system")) {
            String message = ServerQueue.instance.getConfigC().Queue_Perms
                    .replace("%permission%", ChatColor.RED + "queue.system" + ChatColor.GRAY);
            sender.sendMessage(ServerQueue.instance.getConfigC().Prefix + message);
            return true;
        }
        ...
        if (args.length > 0 && args[0].equalsIgnoreCase("on")) {
            ...
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
                    ServerQueue.instance.getConfigC().Prefix + ServerQueue.instance.getConfigC().QueueSystemOnline));
            return true;
        }
        ...
        if (args.length > 0 && args[0].equalsIgnoreCase("off")) {
            ...
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
                    ServerQueue.instance.getConfigC().Prefix + ServerQueue.instance.getConfigC().QueueSystemOffline));
            return true;
        }
